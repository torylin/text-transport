...
\vl{The last fraction of the variance is 0 unless $i=j$, in which case it becomes $$\frac{P^R(X_i,X_i)-P^R(X_i)P^R(X_j)}{P^R(X_i,X_i)} = \frac{P^R(X_i)-P^R(X_i)P^R(X_j)}{P^R(X_i)} = 1-P^R(X_j)$$
which is nonzero.}
...
\vl{The estimates for $\hat{\mu}$ and $\hat{\tau}$ look pretty reasonable, but need to figure out why the variance estimates are so large / why the CIs are so wide.}
...
\vl{Estimates using marginal probabilities with language modeling are pretty unstable. Not sure why?}
...
\begin{table}[!ht]
    ...
    \begin{tabular}{c|cccc}
        ...
        Treatment & $\hat{\mu}(P_1)$ & $\hat{\mu}(P_0)$ & $\hat{\tau}$ & 95\% CI \\
        \midrule
        Commitment & 69.368 & 64.882 & \textcolor{green!50!black}{4.487} & [-7.337, 16.311] \\
        Bravery & 68.013 & 68.229 & \textcolor{red!80!black}{-0.216} & [-10.948, 10.516] \\
        Mistreatment & 70.750 & 67.711 & \textcolor{green!50!black}{3.039} & [-7.638, 13.716] \\
        Flags & 63.571 & 68.671 & \textcolor{red!80!black}{-5.100} & [-19.565, 9.365] \\
        Threat & 66.362 & 69.418 & \textcolor{red!80!black}{-3.055} & [-13.690, 7.579] \\
        Economy & 67.308 & 68.363 & \textcolor{red!80!black}{-1.054} & [-16.479, 14.370] \\
        Violation & 68.099 & 68.209 & \textcolor{red!80!black}{-0.110} & [-10.226, 10.007] \\
        \bottomrule
    \end{tabular}
    \caption{Treatment effect estimates using the language modeling approach, where text probabilities under $P^T$ are estimated from BERT fine-tuned with an MLM task on a small number of held-out samples from $P^T$.}
